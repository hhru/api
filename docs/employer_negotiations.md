...
* [Перенос в «Подумать»](#put-on-hold)
* [Приглашение соискателя в ответ на отклик](#invite)
...
Перенос топика в состояние «Подумать» возможен в случае, если соискатель еще не был приглашен и не получил отказ. Информация о переносе топика в статус «Подумать» не раскрывается соискателю. 

### Запрос

`PUT /negotiations/hold/{topic_id}`

Параметры

Имя | Описание
----|---------
topic_id | идентификатор топика, обязательный параметр
...
----|------|-------|--------
403 | negotiations | wrong_state | топик находится в состоянии, из которого невозможен перенос в состояние «Подумать» (соискатель уже приглашен либо получил отказ)
404 | not_found | — | топик не существует, принадлежит другому работодателю либо у текущего менеджера нет прав для работы с переданным топиком

<a name="invite"/>
## Приглашение соискателя в ответ на отклик

Приглашение возможно, если соискатель еще не был приглашен и не получил отказ. При приглашении соискатель получит письмо на электронную почту.

### Запрос

`PUT /negotiations/invited/{topic_id}`

Параметры

Имя | Тип | Обязательный | Описание
----|-----|--------------|---------
topic_id | строка | да | идентификатор топика
send_sms | логический | нет | уведомлять ли соискателя о приглашении с помощью sms, значение по умолчанию — `false`
message | строка | да | Сообщение, которое будет отправлено соискателю на электронную почту. Используйте [шаблоны писем](./message_templates.md) для получения шаблонов текстов.
address_id | строка | нет | идентификатор адреса, который будет указан в приглашении
...
Код | type | value | Причина
----|------|-------|--------
403 | negotiations | wrong_state | топик находится в состоянии, при котором невозможно приглашение (соискатель уже был приглашен либо получил отказ)
404 | not_found | — | топик не существует, принадлежит другому работодателю либо у текущего менеджера нет прав для работы с переданным топиком
403 | negotiations | resume_not_found | резюме из топика было скрыто либо удалено
400 | bad_argument | empty_mail_text | передан пустой текст письма
400 | bad_argument | too_long_mail_text | Передан слишком длинный текст письма. Максимально допустимая длина письма составляет 10000 символов.
403 | negotiations | address_not_found | переданный адрес не существует либо принадлежит другому работодателю
403 | negotiations | invalid_vacancy | вакансия из топика была заархивирована либо скрыта
403 | negotiations | limit_exceeded | превышен лимит менеджера на количество приглашений в сутки

<a name="discard"/>
## Отказ соискателю

Приглашение возможно, если соискатель еще не был приглашен и не получил отказ. При отказе соискатель получит письмо на электронную почту.

### Запрос 

`PUT /negotiations/discarded/{topic_id}`
...
----|---------
topic_id | идентификатор топика, обязательный параметр
message | Сообщение, которое будет отправлено соискателю на электронную почту. Используйте [шаблоны писем](./message_templates.md) для получения шаблонов текстов.
...
Код | type | value | Причина
----|------|-------|--------
403 | negotiations | wrong_state | топик находится в состоянии, при котором невозможен отказ (соискатель уже был приглашен либо получил отказ)
404 | not_found | — | топик не существует, принадлежит другому работодателю либо у текущего менеджера нет прав для работы с переданным топиком
400 | bad_argument | empty_mail_text | передан пустой текст письма
400 | bad_argument | too_long_mail_text | Передан слишком длинный текст письма. Максимально допустимая длина письма составляет 10000 символов.
