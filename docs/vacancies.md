...
* `text` — текстовое поле.  
  Переданное значение ищется в полях вакансии, указанных в параметре
  `search_field`.  
...
редактировать только целиком, передавая полный объект.  К примеру, для изменения
валюты необходимо передавать обе границы значений, а для изменения специализации
необходимо передать полный список.
