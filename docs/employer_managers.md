...
* [Получение информации о квотах менеджера](#quotas)
...
<a name="quotas"></a>
## Получение информации о квотах менеджера

На менеджеров компании распространяются квоты, которые влияют на количественный доступ к определенным действиям.
На данный момент API поддерживает только квоты по публикации вакансий.

По выдаче можно определить:
* возможные варианты публикации вакансий и соответствующие им запросы к API (`action`)
* квоты менеджера на каждый тип публикации - проверка доступа (`available`) и количественная характеристика (`value`)

### Запрос

`GET /employers/{employer_id}/managers/{manager_id}/quotas`
...
```json
{
    "vacancy_publication": {
        "quotas": [
            {
                "action": {
                    "id": "vacancy_create_open_free",
                    "name": "Опубликовать бесплатную вакансию",
                    "method": "POST",
            ...
            {
                "action": {
                    "id": "vacancy_create_open_standard_plus",
                    "name": "Опубликовать вакансию \"Стандарт плюс: автообновление\"",
                    "method": "POST",
                    "url": "https://api.hh.ru/vacancies",
                    "properties": [
                        {
...
Имя | Тип | Описание
--- | --- | --------
vacancy_publication | object | информация по квотам на публикацию вакансий
vacancy_publication.quotas | array | список [квот](#quota-object)

<a name="quota-object"></a>
Каждый элемент `quotas` содержит:

Имя | Тип | Описание
--- | --- | --------
action | object | [действие в рамках API](#action-object), за которое отвечает квота
value | number | количество доступных действий по квоте
available | boolean | флаг доступности действия по квоте
...
properties | array | поля json-тела для запроса
properties.id | string | название поля
properties.value | any | значение поля для конкретного действия

Поля и их значения можно посмотреть в описании [метода публикации вакансии](employer_managers.md#creation_fields).

### Ошибки

* `403 Forbidden` - текущий пользователь - не работодатель
