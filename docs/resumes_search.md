...
* `text` — поисковая фраза. Найдет резюме, в которых встречаются все слова
  ...
  Специально для этого поля есть [автодополнение](suggests_additional.md#resume-search-keyword).
...
Возврощаемые результаты группируются по соискателю: один и тот же соискатель не может вернуться в выборке несколько раз. 
...
* `403 Forbidden` — ошибка авторизации или отсутствие прав у приложения, отсутствии доступа у работодателя. Дополнительно к коду
   могут вернуться [причины ошибки](https://github.com/hhru/api/blob/master/docs/errors.md#ошибки-доступа-к-платному-методу).
