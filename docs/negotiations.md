...
В элементе `items` содержатся данные о откликах:
 ...
 has_updates | логический | Есть ли непросмотренные сообщения в топике. Флаг сбрасывается при различных действиях по отклику, например, [просмотре списка сообщений](#get_messages).
