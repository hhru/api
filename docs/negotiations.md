...
Возвращаемый объект полностью идентичен отдельному отклику, получаемому в
[списке откликов](#get_negotiations), за исключением поля `messaging_status`.
...
* `403 Bad Request` – если отправить сообщение не удалось
