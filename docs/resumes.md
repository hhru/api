...
Поле `position.professionalRole` выдается только в том случае, когда при запросе передан дополнительный параметр `with_professional_roles`. В этом случае GET запрос должен иметь вид:
