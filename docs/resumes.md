...
(поле `description`), а также должность (поле `position`) доступно только в
...
* `text` — поисковая фраза. Найдет резюме в которых встречаются все слова
  ...
  [язык поисковых запросов](http://hh.ru/article.xml?articleId=1175). Для
  тонкой настройки по фразе можно использовать параметры `text.logic`,
...
* `GET /resumes?text=Headhunter&text.logic=all&text.field=experience&text.period=last_three_years` –
  найдет все резюме,  в опыте работы которых, за последние 3 года,
  встречается 'Headhunter'.
...
* `GET /resumes?text=менеджер%20проекта&text.logic=all&text.field=experience%2Cskills&text.period=last_year&text=ответственный&text.logic=all&text.field=everywhere&text.period=all_time` –
  ...
  встречаются слова 'менеджер' и 'проекта', а также слово 'ответственный' в любом
  месте резюме. Отметим, что дополнительные параметры
...
видимости «видно всему интернету» или «по прямой ссылки» (`everyone` и `direct`
...
`404 Not Found`. Если просмотр контактов не доступен при указанном типе
авторизации, в соответствующих полях будет `null`.
...
это новый элемент, английский,  который мы хотим добавить. Поля `name`, в данном
случае не мешают, но и не несут полезную информацию.
...
Если обновление ещё не доступно — `429 Too Many Requests`.
...
Для клониования можно использовать только собственные резюме, иначе код ответа
будет `404 Not Found`.
 ...
 * Коментарий можно указывать только для телефонов, для email комментарий
   не сохранится.
...
Ключ `status` определяет текущее состояние резюме и содержит элемента
