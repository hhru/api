...
* `text` — поисковая фраза. Найдет резюме в которых встретятся все слова заданной фразы.
...
* `period` — количество дней, в пределах которых нужно найти резюме.
Если не указан, то поиск ведется за весь период.

* `education_level` —  уровень образования.
Справочник с возможными значениями: `education_level` в [/dictionaries](dictionaries.md).
Возможно искать с любым уровнем образования, если не передать этот параметр.
...
При указании параметров пэйджинга (page, per_page) работает ограничение: общее кол-во возвращаемых резюме не может быть больше 5000 и не больше 50 на страницу.
...
* `relocation` — возможен ли переезд в другой город. Состоит из полей:
    ...
    * `area` — город, в который возможен переезд (список). Имеет смысл, только с соответствующим полем `type`. Элемент справочника [areas](areas.md);
...
После того, как резюме публикуется оно переходит в статус `published`. В этом статусе резюме доступно для поиска, если это позволяет [видимость резюме](#access_type).

Когда резюме переходит в статус `published` оно попадает в поле зрения модераторов и может быть заблокировано (статус `blocked`), если поля резюме некорректны или малоиформативны.
Информация о том почему заблокировано резюме можно найти в ключе `moderation_note`. Если резюме заблокировано, то оно пропадает из поиска и не может быть использовано для отклика на вакансию.
После исправления резюме, пользователь может отправить его на повторное рассмотрение модератору.
...
После того как резюме опубликовано, т.е. перешло из статуса `not_published` в `published` его нельзя будет "разпубликовать" (перевести обратно в `not_published`),
...
После публикациии резюме оно доступно для поиска работодателями.
Для того, чтобы это изменить, например, работа найдена и нужно скрыть резюме из поиска, можно поменять видимость резюме.
Видимость определяется в ключе `access.type` у резюме. Тип видимости – элемент справочника [resume_access_type](/dictionaries.md).

 Id  | Описание
 ---- | ---
 no_one | Резюме такого типа никому не будет видно. Но вы можете откликнуться таким резюме на заинтересовавшую вас вакансию. При этом резюме сменит тип на `whitelist`.
 whitelist | Резюме не будет видно никому кроме указанных компаний. Если откликнуться таким резюме на вакансию, работодатель которой не входит в белый список, но данная компания автоматически добавится в этот список.
 blacklist | Резюме будет доступно для поиска и просмотра всеми компаниями за исключением указанных. Если откликнуться на вакансию компании из черного списка, то эта компания вычеркивается из него автоматически.
 clients | Резюме будет доступно всем зарегистрированным компаниям на hh.ru.
 everyone | Резюме будет доступно всем без исключения. Всему интернету и за его пределами.
 direct | Резюме будет видно только по прямой ссылке, которая указана в ключе `alternate_url`.
