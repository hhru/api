...
`GET /resumes/mine` возвращает список резюме авторизованного пользователя.
...
* `403 Forbidden` – ошибка авторизации пользователя. Ожидается соискатель.
...
* `403 Forbidden` - операция публикации резюме недоступна из-за отсутствия прав (например, для работодателя).
...
* `403 Forbidden` – ошибка авторизации пользователя. Ожидается соискатель - автор резюме.
* `404 Not Found` – резюме недоступно.
...
Успешный ответ приходит с кодом `200 OK` и в том же виде, что и [условия заполнения полей резюме](#conditions).
...
* `403 Forbidden` – ошибка авторизации пользователя. Ожидается соискатель - автор резюме.
* `404 Not Found` - резюме не существует.
